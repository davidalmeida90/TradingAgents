...
def test_capital_expenditure_is_found_under_either_tag_filers_use(monkeypatch):
    """Some filers report purchases of productive assets, not of property and equipment."""
    facts = {"facts": {"us-gaap": {"PaymentsToAcquireProductiveAssets": _usd(
        _fact("2024-12-31", 70e6, "2025-02-10", start="2024-01-01", form="10-K"))}}}
    monkeypatch.setattr(sec_edgar, "_fetch_json",
                        lambda url: TICKER_MAP if "company_tickers" in url else facts)
    out = sec_edgar.get_cashflow("ACME", "annual", "2025-03-01")
